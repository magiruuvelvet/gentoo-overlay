...
multilib_src_configure() {
	...
	#export PATH=/usr/musl/bin:$PATH
	#export LD_LIBRARY_PATH=/usr/musl/lib64:$LD_LIBRARY_PATH

	local llvm_version=$(LD_LIBRARY_PATH=/usr/musl/lib64:$LD_LIBRARY_PATH /usr/musl/bin/llvm-config --version) || die
	local clang_version=$(ver_cut 1-3 "${llvm_version}")

	local mycmakeargs=(
		-DCMAKE_INSTALL_PREFIX=/usr/musl
		# ensure that the correct llvm-config is used
		-DLLVM_CONFIG="/usr/musl/bin/llvm-config"
...
multilib_src_compile() {
	...
	# FIXME: /var/tmp/portage/sys-devel/clang-musl-8.0.1/work/x/y/cfe-8.0.1.src-abi_x86_64.amd64/bin/clang-tblgen
	# #!/bin/sh
	# export LD_LIBRARY_PATH=/usr/musl/lib64:$LD_LIBRARY_PATH
	# exec /var/tmp/portage/sys-devel/clang-musl-8.0.1/work/x/y/cfe-8.0.1.src-abi_x86_64.amd64/bin/clang-tblgen.bin "$@"
	#
	# requires several include removals during compilation (mainly CPU performance extensions)
	# must be recompiled later with itself to enable performance extensions again
