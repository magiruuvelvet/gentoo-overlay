...
src_prepare() {
    ...
    elog "Applying distribution patches..."

    # fix wrong or generic paths
    pushd web-$PV_ADMINLTE >/dev/null
    patch -p1 < "${FILESDIR}/5.x/arch-server-admin-5.17.3.patch" || die
    popd >/dev/null

    pushd pi-hole-$PV >/dev/null
    patch -p1 < "${FILESDIR}/5.x/arch-server-core-5.17.3.patch" || die
    popd >/dev/null

    if use doas; then
        elog "Applying patches for doas..."

        pushd web-$PV_ADMINLTE >/dev/null
        patch -p1 < "${FILESDIR}/5.x/5.17.3-web-use-doas-instead-of-sudo.patch" || die
        popd >/dev/null

        pushd pi-hole-$PV >/dev/null
        patch -p1 < "${FILESDIR}/5.x/5.17.3-cli-use-doas-instead-of-sudo.patch" || die
        popd >/dev/null
    fi
